...
<head>
    ...
    <title>Brgy. Caranas — @yield('title', 'Resident Portal')</title>
...
<div class="flex">
    ...
    <aside class="no-print fixed inset-y-0 left-0 z-40 flex w-[220px] flex-col transition-transform duration-300 md:translate-x-0"
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-4">
            <div>
                <p class="px-3 mb-2 text-[10px] font-semibold uppercase tracking-widest text-white/30">Resident Portal</p>
            ...
            {{-- Track a Request --}}
            <div>
                <p class="px-3 mb-2 text-[10px] font-semibold uppercase tracking-widest text-white/30">Quick Tools</p>
                <form action="{{ route('resident.documents.track') }}" method="GET" class="px-1">
                    <div class="flex gap-1">
                        <input type="text" name="tracking_number" placeholder="Tracking #"
                               class="flex-1 rounded-lg bg-white/10 border border-white/20 text-white text-xs px-2.5 py-2 placeholder-white/30 focus:outline-none focus:ring-1 focus:ring-white/40 min-w-0">
                        <button type="submit"
                                class="shrink-0 rounded-lg bg-white/15 border border-white/20 px-2 py-2 text-white hover:bg-white/25 transition-colors">
        ...
        <div class="border-t border-white/10 px-3 py-4 space-y-2">
            <div class="flex items-center gap-3 px-2">
                ...
                <div class="min-w-0 flex-1">
                    <p class="truncate text-xs font-semibold text-white">{{ Auth::user()->name ?? 'Resident' }}</p>
                    <div class="flex items-center gap-1 mt-0.5">
                        <span class="inline-flex items-center rounded-full bg-sky-400/20 border border-sky-400/30 px-1.5 py-0.5 text-[9px] font-semibold text-sky-300">
                            <i class="fa-solid fa-user mr-0.5 text-[8px]"></i> RESIDENT
                        </span>
                    </div>
                </div>
            </div>
