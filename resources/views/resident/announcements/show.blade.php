...
<div class="mb-5">
    <a href="{{ route('resident.announcements.index') }}"
       class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-green-700 transition-colors">
        <i class="fa-solid fa-chevron-left text-xs"></i> Back to Announcements
    </a>
</div>
    ...
    <div class="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-6 border-b border-gray-100" style="background: linear-gradient(135deg, #1a4731 0%, #2d6a4f 100%);">
            <div class="flex items-center gap-2 mb-3">
                ...
                <span class="text-white/50 text-xs">
                    <i class="fa-solid fa-users text-[10px] mr-1"></i>{{ $announcement->audience ?? 'All Residents' }}
                </span>
            </div>
            <h1 class="text-xl font-bold text-white leading-snug">{{ $announcement->title }}</h1>
            <p class="text-white/60 text-xs mt-2">
                <i class="fa-regular fa-calendar mr-1"></i>
                Posted {{ $announcement->published_at?->format('F j, Y') }}
                @if($announcement->expires_at)
                    &middot; Expires {{ $announcement->expires_at->format('F j, Y') }}
                @endif
            </p>
        </div>
        ...
        <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
            <span class="text-xs text-gray-400">
                Barangay Caranas Official Announcement
            </span>
            <a href="{{ route('resident.announcements.index') }}"
               ...
               onmouseout="this.style.backgroundColor='#1a4731'">
                <i class="fa-solid fa-arrow-left text-[10px]"></i> Back to Board
            </a>
        </div>
    </div>
</div>
