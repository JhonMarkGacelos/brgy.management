...
<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-lg font-bold text-gray-900">Welcome, {{ explode(' ', Auth::user()->name ?? 'Resident')[0] }}!</h2>
        <p class="text-xs text-gray-400 mt-0.5">{{ now()->format('l, F j, Y') }} &mdash; Barangay Caranas Resident Portal</p>
    </div>
    <a href="{{ route('resident.documents.create') }}"
       ...
       onmouseout="this.style.backgroundColor='#1a4731'">
        <i class="fa-solid fa-file-circle-plus text-xs"></i> Request Document
    </a>
</div>
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
    @php
        $cards = [
            ['label'=>'Pending Requests',  'value'=>$stats['pending'],  'sub'=>'Awaiting processing',   'icon'=>'fa-clock',             'iconBg'=>'bg-amber-100', 'iconColor'=>'text-amber-600'],
            ['label'=>'Approved',          'value'=>$stats['approved'], 'sub'=>'Ready for release',     'icon'=>'fa-circle-check',      'iconBg'=>'bg-green-100', 'iconColor'=>'text-green-600'],
            ['label'=>'Issued',            'value'=>$stats['issued'],   'sub'=>'Documents released',    'icon'=>'fa-file-circle-check', 'iconBg'=>'bg-blue-100',  'iconColor'=>'text-blue-600'],
        ];
    @endphp
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
    ...
    <div class="lg:col-span-2 rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <div class="flex items-center gap-2">
                ...
                <p class="text-sm font-semibold text-gray-900">My Recent Requests</p>
            </div>
            <a href="{{ route('resident.documents.index') }}" class="text-xs font-medium text-green-700 hover:underline">View all</a>
        </div>
        @php
            $statusColors = [
        ...
        <div class="divide-y divide-gray-50">
            ...
            <div class="px-5 py-10 text-center text-gray-400 text-sm">
                ...
                No requests yet. <a href="{{ route('resident.documents.create') }}" class="text-green-700 font-medium hover:underline">Make your first request.</a>
            </div>
            @endforelse
        </div>
    </div>
    ...
    <div class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-2.5">
                <a href="{{ route('resident.documents.create') }}"
                   class="group flex flex-col items-center justify-center gap-2 rounded-xl p-4 bg-green-50 hover:bg-green-700 transition-all duration-200">
                    ...
                    <span class="text-xs font-semibold text-green-800 group-hover:text-white transition-colors leading-tight text-center">Request Document</span>
                </a>
                ...
                <a href="{{ route('resident.announcements.index') }}"
                   class="group flex flex-col items-center justify-center gap-2 rounded-xl p-4 bg-purple-50 hover:bg-purple-600 transition-all duration-200">
                    ...
                    <span class="text-xs font-semibold text-purple-600 group-hover:text-white transition-colors leading-tight text-center">Announcements</span>
                </a>
                ...
                <a href="{{ route('resident.documents.index') }}"
                   class="group flex flex-col items-center justify-center gap-2 rounded-xl p-4 bg-blue-50 hover:bg-blue-600 transition-all duration-200 col-span-2">
                    ...
                    <span class="text-xs font-semibold text-blue-600 group-hover:text-white transition-colors leading-tight text-center">View All My Requests</span>
                </a>
            </div>
        </div>
        ...
        {{-- Track Request --}}
        <div class="rounded-2xl bg-white border border-gray-100 shadow-sm p-5">
            <p class="text-sm font-semibold text-gray-900 mb-1">Track a Request</p>
            <p class="text-xs text-gray-400 mb-4">Enter your tracking number to check its status.</p>
            <form action="{{ route('resident.documents.track') }}" method="GET" class="space-y-3">
                ...
                <button type="submit"
                        ...
                        onmouseover="this.style.backgroundColor='#2d6a4f'"
                        onmouseout="this.style.backgroundColor='#1a4731'">
                    <i class="fa-solid fa-magnifying-glass mr-1.5 text-xs"></i> Track
                </button>
            </form>
        </div>
    </div>
...
<div class="mt-5 rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <div class="flex items-center gap-2">
            ...
            <p class="text-sm font-semibold text-gray-900">Latest Announcements</p>
        </div>
        <a href="{{ route('resident.announcements.index') }}" class="text-xs font-medium text-green-700 hover:underline">View all</a>
    </div>
    <div class="divide-y divide-gray-50">
