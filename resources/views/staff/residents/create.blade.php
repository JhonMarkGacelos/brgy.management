...
<div class="flex items-center gap-3 mb-6">
    ...
    <div>
        <h2 class="text-lg font-bold text-gray-900">Register New Household</h2>
        <p class="text-xs text-gray-400 mt-0.5">Fill in the shared address and add one or more family units</p>
    </div>
</div>
        ...
        <div class="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="p-5 space-y-3">
                <button type="submit"
                    ...
                    <i class="fa-solid fa-floppy-disk text-xs"></i> Register Household
                </button>
                <a href="{{ route('staff.residents.index') }}"
                ...
                <template x-for="(family, fi) in families" :key="fi">
                    <div class="flex items-center justify-between pl-3 border-l-2" :style="'border-color:' + familyColor(fi)">
                        ...
                        <span class="text-xs font-semibold text-gray-700"
                              x-text="(1 + family.members.length) + ' member' + (family.members.length !== 0 ? 's' : '')"></span>
                    </div>
                </template>
