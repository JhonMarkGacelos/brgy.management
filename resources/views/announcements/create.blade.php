...
@section('title', isset($announcement) ? 'Edit Announcement' : 'Post Announcement')
...
<div class="flex items-center gap-3 mb-6">
    ...
    <div>
        <h2 class="text-base font-semibold text-gray-900">{{ isset($announcement) ? 'Edit Announcement' : 'Post Announcement' }}</h2>
        <p class="text-xs text-gray-400 mt-0.5">{{ isset($announcement) ? 'Update announcement details' : 'Create a new community notice or announcement' }}</p>
    </div>
</div>

<form action="{{ isset($announcement) ? route($isStaff ? 'staff.announcements.update' : 'announcements.update', $announcement->id) : route($isStaff ? 'staff.announcements.store' : 'announcements.store') }}" method="POST">
@csrf
            ...
            <div class="p-5 space-y-4">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-5 gap-2">
                        ...
                        <label class="group flex flex-col items-center gap-1.5 rounded-xl border-2 border-gray-200 bg-white p-3 cursor-pointer transition-all
                            ...
                            <input type="radio" name="category" value="{{ $cat['value'] }}"
                                   class="sr-only" {{ (old('category', isset($announcement) ? $announcement->category : '')) === $cat['value'] ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="content" rows="8" placeholder="Write the full announcement here..."
                              class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3.5 py-2.5 text-sm text-gray-900
                                     focus:border-green-600 focus:ring-2 focus:ring-green-600/20 focus:bg-white focus:outline-none transition-all resize-none placeholder-gray-400">{{ old('content', isset($announcement) ? $announcement->content : '') }}</textarea>
                </div>
        ...
        <div class="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="published_at" value="{{ old('published_at', isset($announcement) ? $announcement->published_at?->format('Y-m-d') : date('Y-m-d')) }}"
                           class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3.5 py-2.5 text-sm text-gray-900
                ...
                <div>
                    ...
                    <input type="date" name="expires_at" value="{{ old('expires_at', isset($announcement) ? $announcement->expires_at?->format('Y-m-d') : '') }}"
                           class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3.5 py-2.5 text-sm text-gray-900
                                  focus:border-green-600 focus:ring-2 focus:ring-green-600/20 focus:bg-white focus:outline-none transition-all">
        ...
        <div class="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="p-5 space-y-3">
                <button type="submit"
                    ...
                    <i class="fa-solid fa-bullhorn text-xs"></i> {{ isset($announcement) ? 'Update Announcement' : 'Post Announcement' }}
                </button>
                <a href="{{ route($isStaff ? 'staff.announcements.index' : 'announcements.index') }}"
        ...
        <div class="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="p-5">
                <div class="flex items-center justify-between">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer" x-data="{ pinned: {{ old('is_pinned') ? 'true' : 'false' }} }">
        ...
        <div class="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="p-5">
                ...
                <select name="audience"
                        class="w-full rounded-xl border border-gray-200 bg-gray-50 px-3.5 py-2.5 text-sm text-gray-900
                               focus:border-green-600 focus:ring-2 focus:ring-green-600/20 focus:bg-white focus:outline-none transition-all">
                    <option value="All Residents" {{ old('audience', isset($announcement) ? $announcement->audience : 'All Residents') === 'All Residents' ? 'selected' : '' }}>All Residents</option>
                    <option value="Senior Citizens" {{ old('audience', isset($announcement) ? $announcement->audience : '') === 'Senior Citizens' ? 'selected' : '' }}>Senior Citizens</option>
                    <option value="4Ps" {{ old('audience', isset($announcement) ? $announcement->audience : '') === '4Ps' ? 'selected' : '' }}>4Ps Members</option>
                    <option value="PWD" {{ old('audience', isset($announcement) ? $announcement->audience : '') === 'PWD' ? 'selected' : '' }}>PWD</option>
                    <option value="Voters" {{ old('audience', isset($announcement) ? $announcement->audience : '') === 'Voters' ? 'selected' : '' }}>Voters</option>
                </select>
            </div>
        </div>
